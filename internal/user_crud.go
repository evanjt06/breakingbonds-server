// Code Generated By gen-mysqlcrud For "Struct Type: User" - DO NOT EDIT;
...
//		Points sql.NullInt32 = REQUIRED
func (r *User) Fill(Email string, Password string, Points sql.NullInt32) {
	r.Email = Email
	r.Password = Password
	r.Points = Points
}
...
// GetPoints retrieves struct field value,
// if any field data type is int representing enum, then the corresponding enum is returned,
// if any field data type is sql null type or pointer, then the null or pointer is converted to base type
func (r *User) GetPoints() int {
	return util.FromNullInt(r.Points)
}
...
func (r *User) SetEmail(v string) error {
	if util.LenTrim(v) == 0 {
...
// SetPoints sets value into struct field value and checks for data input initial validation rules,
// if any field data type is int representing enum, then the corresponding enum is used as input parameter,
// if any field data type is sql null type or pointer, then the base type is used as parameter and converted into null or pointer
//		[ Parameters ]
//		v int = Struct internal data type: sql.NullInt32; REQUIRED
func (r *User) SetPoints(v int) error {
	r.Points = util.ToNullInt(v, true)
...
// GetByEmail retrieves a row from database using the Email field value,
// and marshals found result fields into struct,
// an error of nil indicates success
// note: this function will have struct tag ggAttr:"9 = GetBy" for columns acting as its source
...
func (r *User) Set() error {
	...
	if r.ID <= 0 {
		...
		q.Set("(Email, Password, Points) ")
		q.Set("VALUES ")
		q.Set("(:Email, :Password, :Points);")

		q.Named("Email", r.Email)
		q.Named("Password", r.Password)
		q.Named("Points", r.Points)
	} else {
		// update
		isNewRow = false
		...
		q.Set("UPDATE User ")
		q.Set("SET Email=:Email, Password=:Password, Points=:Points ")
		q.Set("WHERE ID=:ID;")
