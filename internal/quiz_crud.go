// Code Generated By gen-mysqlcrud For "Struct Type: Quiz" - DO NOT EDIT;
...
// GetByPacketNumber retrieves a row from database using the PacketNumber field value,
// and marshals found result fields into struct,
// an error of nil indicates success
// note: this function will have struct tag ggAttr:"9 = GetBy" for columns acting as its source
func (r *Quiz) GetByPacketNumber(PacketNumber int) (notFound bool, err error) {
	// clean up
	r._originalValue = nil
	...
	q.Set("SELECT * FROM Quiz WHERE PacketNumber=? LIMIT 1;")
	q.Ordinal(PacketNumber)
...
// GetByPacketNumber will load struct slice for one or more matching values of same parameter from database
// [ Parameters ]
//		PacketNumberToLoad = variadic, one or more table PacketNumber to load from database, for example, enter 2, 3, 6, 9, will load records with PacketNumber 2, 3, 6, and 9 from database to struct list
// [ Notes ]
// 		this function is based on the database column dbAttr:"GetBy" for suffix naming
func (l *QuizList) GetByPacketNumber(PacketNumberToLoad ...int) error {
	// validate
	if l == nil {
		return errors.New("List Object Nil")
	}

	if PacketNumberToLoad == nil {
		return errors.New("Get Requires One or More Filter Values (Input Nil)")
	}

	if len(PacketNumberToLoad) == 0 {
		return errors.New("Get Requires One or More Filter Values (Input Count 0)")
	}
	...
	for _, v := range PacketNumberToLoad {
		if util.LenTrim(filter) > 0 {
			filter += ", "
		}
	...
	if strings.Contains(filter, ",") {
		filter = "WHERE PacketNumber IN (" + filter + ")"
	} else {
		filter = "WHERE PacketNumber = " + filter
	}
...
// FindByPacketNumber will search existing struct slice for one or more matching values of same parameter,
// it will return a list of found objects, or error if encountered
// [ Parameters ]
//		PacketNumberToFind = variadic, one or more table PacketNumber to find, for example, enter 2, 3, 6, 9, will find records with PacketNumber 2, 3, 6, and 9 in list
// [ Notes ]
//		this function is based on the database column dbAttr:"GetBy" for suffix naming
func (l *QuizList) FindByPacketNumber(PacketNumberToFind ...int) (foundList *[]Quiz, err error) {
	// validate
	if l == nil {
		// if not valid, then return error
		return nil, errors.New("List Object Nil")
	...
	// loop thru list to match
	for _, seek := range PacketNumberToFind {
		if seek > 0 {
			for _, v := range *l.List {
				if v.PacketNumber != 0 {
					if v.PacketNumber == seek {
						// found match
						*foundList = append(*foundList, v)
					}
				}
