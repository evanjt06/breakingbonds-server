...
func (r *Admin) Fill(Email string, Password string) {
	r.Email = Email
	r.Password = Password
}

// GetEmail retrieves struct field value,
...
// if any field data type is int representing enum, then the corresponding enum is returned,
// if any field data type is sql null type or pointer, then the null or pointer is converted to base type
func (r *Admin) GetPassword() string {
	return r.Password
}

// SetEmail sets value into struct field value and checks for data input initial validation rules,
...
func (r *Admin) SetEmail(v string) error {
	if util.LenTrim(v) == 0 {
		return errors.New("SetEmail Failed, A Text Length Greater Than Zero is Required")
	}
...
//		[ Parameters ]
//		v string = Struct internal data type: string; REQUIRED
func (r *Admin) SetPassword(v string) error {
	if util.LenTrim(v) == 0 {
...
func (r *Admin) GetByID(ID int64) (notFound bool, err error) {
	// clean up
	...
	q.Set("SELECT * FROM Admin WHERE ID=? LIMIT 1;")
	q.Ordinal(ID)

	// execute query
	var dbCurrent *data.MySql

	if !_AdminPrefersDBReader {
		dbCurrent = db
	} else {
...
func (r *Admin) GetScalar(scalarColumnName string, filter string, sort string, args ...interface{}) (result string, notFound bool, err error) {
	// validate
	...
	if !isFromOverride {
		q.Set("SELECT " + scalarColumnName + " FROM Admin " + filter + sort + " LIMIT 1;")
	} else {
...
func (r *Admin) IsDataChanged() bool {
...
func (r *Admin) Set() error {
	...
	if r.ID <= 0 {
		...
		q.Set("INSERT INTO Admin ")
		q.Set("(Email, Password) ")
		q.Set("VALUES ")
		q.Set("(:Email, :Password);")

		q.Named("Email", r.Email)
		...
		q.Set("UPDATE Admin ")
		q.Set("SET Email=:Email, Password=:Password ")
		q.Set("WHERE ID=:ID;")
	...
	if tx, e = db.Begin(); e != nil {
		return errors.New("Admin Persist Data Failed at BeginTran: " + e.Error())
	}
	...
	if result.Err != nil {
		if err1 := tx.Rollback(); err1 != nil {
			return errors.New("Admin Persist Data Failed at RollBack 1: " + err1.Error() + " (Orig Error: " + result.Err.Error() + ")")
		} else {
			return errors.New("Admin Persist Data Failed: " + result.Err.Error())
		}
	}
	...
	if !isNewRow {
		...
		if result.RowsAffected <= 0 {
			// update failure
			if err1 := tx.Rollback(); err1 != nil {
				return errors.New("Admin Persist Data Failed at RollBack 2: " + err1.Error() + " (Orig Error: No Rows Affected)")
			} else {
				return errors.New("Admin Persist Updated Data Failed: No Rows Affected")
			}
		}
	} else {
		// was insert
		...
		if result.NewlyInsertedID > 0 {
			...
			if err1 := tx.Rollback(); err1 != nil {
				return errors.New("Admin Persist Data Failed at RollBack 3: " + err1.Error() + " (Orig Error: No New Primary ID Generated)")
			} else {
				return errors.New("Admin Persist Inserted Data Failed: No New Primary ID Generated")
			}
		}
	}
	...
	if err1 := tx.Commit(); err1 != nil {
		return errors.New("Admin Persist Data Failed at Commit: " + err1.Error())
	}
...
func (r *Admin) physicalDelete() error {
	// compose query
	q := new(data.QueryBuilder)

	q.Set("DELETE FROM Admin WHERE ID=?;")
	q.Ordinal(r.ID)
	...
	if result.Err != nil {
		return errors.New("Admin Physical Delete Failed: " + result.Err.Error())
	}

	if result.RowsAffected <= 0 {
		return errors.New("Admin Physical Delete Failed: No Rows Affected")
	}
...
func (r *Admin) Delete() error {
	// validate
	if r.ID <= 0 {
		return errors.New("Admin Delete Data Failed: Row Primary ID is Required")
	}

	if r._originalValue == nil {
		return errors.New("Admin Delete Data Failed: Row Must Be Loaded First")
	}
...
// AdminList collection level dal model
type AdminList struct {
	List  *[]Admin
	Count int
}

// indicates if Admin list will prefer dbReader over dbWriter where available
var _AdminListPrefersDBReader bool

// UseDBReaderPreferred sets indicator that this struct prefers to use db reader where applicable and if available
func (l AdminList) UseDBReaderPreferred() {
	// set db reader usage preference to true, so if db reader is available, we will use it if code is applicable for it
	_AdminListPrefersDBReader = true
}

// UseDBWriterPreferred sets indicator that this struct prefers to use db writer (this is the default preference)
func (l AdminList) UseDBWriterPreferred() {
	// set db writer usage preference to true, so this struct list will use the db writer for operations
	_AdminListPrefersDBReader = false
}

// Element gets one of the slice elements in list, identified by ordinal index position
func (l *AdminList) Element(position int) (*Admin, error) {
	// validate
	if l == nil {
...
func (l *AdminList) GetAll(limitCount int, offsetCount ...int) error {
...
func (l *AdminList) GetByID(IDToLoad ...int64) error {
	// validate
	if l == nil {
...
func (l *AdminList) GetCustom(filter string, sort string, offsetCount int, limitCount int, args ...interface{}) error {
	// validate
	if l == nil {
		return errors.New("List Object Nil")
	}
...
//		args = optional ordinal parameter arguments to pass into the filter query portion (must appear in ordinal position of params defined within filter)
func (l *AdminList) getInternal(filter string, sort string, offsetCount int, limitCount int, args ...interface{}) error {
	// validate
	if l == nil {
		return errors.New("List Object Nil")
	}
	...
	if !isFromOverride {
		q.Set("SELECT * FROM Admin " + filter + sort + limit + ";")
	} else {
...
func (l *AdminList) IsDataChanged() (changedList *[]Admin) {
	// validate
	if l == nil {
...
func (l *AdminList) Validate() (invalid *Admin, invalidInfo string, err error) {
	// validate
	if l == nil {
...
// a new database row will be inserted if such was not found,
// otherwise existing database row will be updated
func (l *AdminList) Set() (failed *Admin, err error) {
	// validate
	if l == nil {
		// if not valid, then return error
		return nil, errors.New("List Object Nil")
...
func (l *AdminList) Delete() (failed *Admin, err error) {
	// validate
	if l == nil {
		// if not valid, then return error
		return nil, errors.New("List Object Nil")
...
// [ Notes ]
//		this function is based on the database column dbAttr:"GetBy" for suffix naming
func (l *AdminList) FindByID(IDToFind ...int64) (foundList *[]Admin, err error) {
	// validate
	if l == nil {
		// if not valid, then return error
		return nil, errors.New("List Object Nil")
